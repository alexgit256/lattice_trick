...
from lattices import generate_close_vectors_my, generate_close_vectors_old, generate_small_norm_quat
from setup import (
    O0,
    p,
    q,
...
def strong_approximation_construct_lattice(N, C, D, λ, L, small_power_of_two=False, seed=0, which_enum="my"):
    """
...
def strong_approximation_lattice_heuristic(N, C, D, λ, L, small_power_of_two=False, seed=0, which_enum="my", dump=True):
    """
    ...
    if which_enum=="my":
        close_vectors = generate_close_vectors_my(lattice_basis, -target, p, L, seed=seed, which_enum=which_enum)
    else:
        close_vectors = generate_close_vectors_old(lattice_basis, -target, p, L, seed=seed, which_enum=which_enum)
...
def StrongApproximationHeuristic(
    ...
    if composite_factors:
        ...
        return strong_approximation_lattice_heuristic(
            N, C, D, λ, L2, small_power_of_two=small_power_of_two, seed=randint(0,2**16-1)
        )
